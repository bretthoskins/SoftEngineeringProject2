...
void Autocomplete::search(string key, int& firstIndex, int& lastIndex)
{
	firstIndex = -1;
	lastIndex = -1;
	int keyIndex = binary_search(key);

	for(int i = keyIndex; i >= 0; i--){
		if(terms[i].compareByPrefix(terms[i],terms[keyIndex],key.length())!=0){
			firstIndex = i+1;
			break;
		}
	}

	for(int i = keyIndex; i < terms.size(); i++){
		if(terms[i].compareByPrefix(terms[i],terms[keyIndex],key.length())!=0){
			lastIndex = i-1;
			break;
		}
...
{
	SortingList<Term> correctTerms;
	int firstIndex;
	int lastIndex;
	search(prefix, firstIndex, lastIndex);
	for(int i = firstIndex; i <= lastIndex; i++){
		correctTerms.insert(this->terms[i]);
	} 
	correctTerms.merge_sort(Term::compareByWeight);
